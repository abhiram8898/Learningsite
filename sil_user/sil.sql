...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 11, 2023 at 06:38 AM
...
(1, 'pbkdf2_sha256$600000$sqw8Rc35iGW6gV37TQ5R00$LD9pafawPUF96nJk9XKvetadZQfZ3hP8Dzbd0qrBdGs=', '2023-04-04 09:44:44.142342', 1, 'admin', '', '', 'admin@gmail.com', 1, 1, '2023-04-04 09:44:15.175791');
...
('6t7hitwrzlj9ta2nzpuago9fk81xvipa', '.eJxVjMsOwiAQRf-FtSG8KS7d-w2EmQGpGkhKuzL-uzbpQrf3nHNfLKZtrXEbeYkzsTOT7PS7QcJHbjuge2q3zrG3dZmB7wo_6ODXTvl5Ody_g5pG_dbKCHASQCIAepoEFlDWpyK11cpqIwsRThi8pqCcwaKt8C5kAQGzkOz9Ae4TN_E:1pjdDY:NQKcgyo5H34LLYF6WZ60Fv_e16lr7udrJnPHBHXLcSE', '2023-04-18 09:44:44.145943');

-- --------------------------------------------------------

--
-- Table structure for table `sil_user_cart`
--

CREATE TABLE `sil_user_cart` (
  `id` bigint(20) NOT NULL,
...
(8, '100 Days Of Code', 'Learn web development from A to Z in 100 days (or at your own pace) - from \"basic\" to \"advanced\", it\'s all included!', 'courses/pngwing.com_1.png', 0, 'Free', 3, 1),
...
(11, 'Customer Service Training', 'This Customer Service training course teaches the basics of customer service and translates that knowledge into practical application. Its a wonderful course to have.', 'courses/customer.png', 449, 'paid', 3, 2),
(12, 'Accounting & Bookkeeping', 'Learn both bookkeeping fundamentals and advanced tips on how to correctly manage your business’s money. Real-world experts will show you the ins and outs of bookkeeping through easy, step-by-step instructions and hands-on exercises.', 'courses/accounting__bookkeeping.png', 0, 'free', 6, 3),
...
(14, 'Investing & Trading', 'By investing your money, you can increase your personal wealth and ensure financial stability for you and your family. Investing courses can teach you the nuances of financially growing your own monetary worth.', 'courses/investment.png', 799, 'paid', 6, 3),
(15, 'Network & Security', 'Find a cyber security course on Udemy, and gain skills to help you counter cyber threats and grow as a cyber security specialist. With cyber security training, you can develop expertise that is expected to be in demand well into the future. Cyber', 'courses/pngwing.com_5.png', 0, 'free', 3, 4),
...
(17, 'Operating Systems & Servers', 'The course starts with an introduction of Operating System and covers the topics such as Types of Operating System, process and their management, CPU Scheduling Algorithm, Synchronization and their need, Deadlock, Memories and their management, and File Systems.', 'courses/pngwing.com_6.png', 449, 'paid', 5, 4);
...
--
-- Indexes for table `sil_user_cart`
--
ALTER TABLE `sil_user_cart`
  ADD PRIMARY KEY (`id`),
  ADD KEY `sil_user_cart_user_id_id_35021336_fk_auth_user_id` (`user_id_id`),
  ADD KEY `sil_user_cart_course_id_id_007d6fac_fk_sil_user_course_id` (`course_id_id`);
...
--
-- AUTO_INCREMENT for table `sil_user_cart`
--
ALTER TABLE `sil_user_cart`
  MODIFY `id` bigint(20) NOT NULL AUTO_INCREMENT;
...
--
-- Constraints for table `sil_user_cart`
--
ALTER TABLE `sil_user_cart`
  ADD CONSTRAINT `sil_user_cart_course_id_id_007d6fac_fk_sil_user_course_id` FOREIGN KEY (`course_id_id`) REFERENCES `sil_user_course` (`id`),
  ADD CONSTRAINT `sil_user_cart_user_id_id_35021336_fk_auth_user_id` FOREIGN KEY (`user_id_id`) REFERENCES `auth_user` (`id`);
